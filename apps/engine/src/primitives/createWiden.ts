...
export const createWiden = async ({
  ...
  const signals = head.result.value.signals.filter((s) => {
    return s.text !== mutation;
  });

  if (signals.length === head.result.value.signals.length) {
    throw new Error(`No signal to retract was found with text ${mutation}`);
  }
