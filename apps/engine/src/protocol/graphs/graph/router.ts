...
import { generate as generateNodes } from "./nodes/generate.js";
...
router.post("/:id/edges/generate", async (req, res) => {
...
const zGenerateNodesParams = z.object({
  id: z.string().uuid(),
});

router.post("/:id/nodes/generate", async (req, res) => {
  const params = zGenerateNodesParams.parse(req.params);
  const data = await generateNodes({ id: params.id });
